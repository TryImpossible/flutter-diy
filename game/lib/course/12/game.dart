...
class OwnGame extends FlameGame
    with KeyboardEvents, PanDetector, DoubleTapDetector {
  ...
  void _checkAttackHero() {
    final Iterable<AnimBullet> bullets = children.whereType<AnimBullet>().where(
      (AnimBullet e) => e.type == BulletType.monster,
    );
  ...
  void _checkAttackMonster() {
    final Iterable<Bullet> bullets = children.whereType<Bullet>();
    final Iterable<Monster> monsters = monsterManager.children
        .whereType<Monster>();
