...
class OwnGame extends FlameGame
  ...
  Future<void> addPlayer() async {
    List<Sprite> sprites = [];
    for (int i = 0; i <= 8; i++) {
      final String src = Assets.images.adventurer.adventurerBow00.keyName
          .replaceFirst(RegExp('00'), '0$i');
      sprites.add(await loadSprite(src));
    }
    ...
    Sprite bulletSprite = await loadSprite(
      Assets.images.adventurer.weaponArrow.keyName,
    );
  ...
  Future<void> addMonster() async {
    final String src = Assets.images.adventurer.animatronic.keyName;
    ...
    for (var i = 0; i <= 28; i++) {
      final String src = Assets.images.adventurer.skill02.s0.keyName
          .replaceFirst(RegExp('s0'), 's$i');
      bossBulletSprites.add(await loadSprite(src));
    }
    ...
    final String src2 = Assets.images.adventurer.charactersPart2.keyName;
    ...
    for (var i = 1; i <= 4; i++) {
      final String src = Assets.images.adventurer.skill01.ef01.keyName
          .replaceFirst(RegExp('ef01'), 'ef${i.toString().padLeft(2, '0')}');
      stoneBulletSprites.add(await loadSprite(src));
    }
