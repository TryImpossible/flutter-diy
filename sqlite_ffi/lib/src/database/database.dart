...
import 'package:sqlite_ffi/src/database/closable_iterator.dart';
import 'package:sqlite_ffi/src/third_party/sqlite_ffi_bindings_generated.dart';

final sqlite = SqliteFfiBindings(DynamicLibrary.open('libsqlite3.dylib'));
...
class Database {
  ...
  Database(
    ...
    final resultCode = sqlite.sqlite3_open_v2(
      pathC.cast(),
      dbOut,
      flags,
      nullptr,
  ...
  SQLiteException _loadError(int errorCode) {
    final errorMessage = sqlite
        .sqlite3_errmsg(_database)
        .cast<Utf8>()
        .toDartString();
    final errorCodeExplanation = sqlite
...
class Row {
  ...
  dynamic operator [](String columnName) {
    ...
    final dynamicType = _typeFromCode(
      sqlite.sqlite3_column_type(_statement, columnIndex),
    );
